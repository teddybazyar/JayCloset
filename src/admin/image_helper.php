...
function fixImageOrientation($image, $filePath) {
    ...
    switch ($orientation) {
        ...
        case 6:
            // Rotate 90 degrees clockwise (or -90 counter-clockwise)
            $image = imagerotate($image, -90, 0);
            break;
            
        case 8:
            // Rotate 90 degrees counter-clockwise (or -270 clockwise)
            $image = imagerotate($image, 90, 0);
            break;
    }
...
function convertImageToPng($sourcePath, $destinationPath, $extension) {
    ...
    // Fix orientation for JPEG images (most common source of rotation issues)
    if (in_array(strtolower($extension), ['jpg', 'jpeg'])) {
        $image = fixImageOrientation($image, $sourcePath);
    }
    ...
    // Save as PNG with moderate compression (5 instead of 9 for faster processing)
    $success = imagepng($image, $destinationPath, 5);
